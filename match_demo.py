...
def demo():
    ...
    atc.add_method_call(app_id, add_method, acct.address, sp, acct.signer, [3, 4])
    results: AtomicTransactionResponse = atc.execute(algod, 4)
    print(f"Confirmed in round {results.confirmed_round}")

    # Make sure we got what we expect (3+4 == 7)
    assert results.abi_results[0].return_value == 7
